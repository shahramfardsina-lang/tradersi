...
class MainWindow(QMainWindow):
    ...
    def process_rsi_batch(self):
        ...
        if not getattr(self, "coins_data", None):
            return

        start = self.rsi_batch_index * self.rsi_batch_size
        end = min(start + self.rsi_batch_size, len(self.coins_data))

        self.status.setText(f"RSI Batch {(self.rsi_batch_index + 1)}/5")

        for row in range(start, end):

            try:
                coin = self.coins_data[row]
                prices, error = get_candles(coin["name"].lower())
                ...
                if prices:
                    ...
                    self.table.setItem(
                        row,
                        4,
                        QTableWidgetItem(str(rsi))
                    )
            except Exception:
                pass

        self.rsi_batch_index += 1

        if start >= 80 or self.rsi_batch_index >= 5:
            self.rsi_batch_index = 0
